...
local function generate_matrix_two(probability_singular)
    --- returns {a, b, d, e}
    -- Generate random values for the matrix
    local a = math.random(-10, 10)
    local b = math.random(-10, 10)
    local d = math.random(-10, 10)
    local e = math.random(-10, 10)
    
    -- Check if the determinant is non-zero
    local determinant = a * e - b * d

    -- Determine if the matrix should be singular or invertible based on probability
    local is_singular = math.random() < probability_singular
        
    if is_singular and determinant==0 then
         -- If matrix should be singular and is singular, return singular matrix
        return {{a, b}, {d, e}}

    elseif determinant==0 then 
        return generate_matrix_two(probability_singular)
    else 
        -- Return the matrix
        return {{a, b}, {d, e}}
    end
...
local function number_solutions_two(eqs)
    ...
    if a1/a2 == b1/b2 and b1/b2 == c1/c2 then
        num_sols = math.huge -- Coincident lines, infinitely many solutions
    
    -- Check if the lines are parallel
    elseif a1/a2 == b1/b2 and a1/a2 ~= c1/c2 then
        num_sols= 0 -- Parallel lines, no solutions

    else num_sols=1 -- lines are intersecting
...
local function generate_exercise(probability_singular)
    local matrix = generate_matrix_two(probability_singular)
    local system = generate_linear_system(matrix)
